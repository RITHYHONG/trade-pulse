...
export async function initializeUserProfile(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('permission')) {
      console.log('Permission error during initialization - this is normal for new users');
    }
...
export async function uploadProfileImage(userId: string, file: File): Promise<string> {
  try {
    ...
    // Use base64 fallback by default until Firebase Storage is properly configured
    console.log('Using base64 fallback for image storage');
    
    // Reduce file size limit for base64 storage
    const maxSizeForBase64 = 1 * 1024 * 1024; // 1MB for base64
    ...
    // TODO: Uncomment this section once Firebase Storage CORS is fixed
    /*
    ...
    try {
      ...
      console.log('Uploading image to:', `profile-images/${userId}/${fileName}`);
      ...
      console.warn('Firebase Storage not available, using base64 fallback:', storageError);
      
      // Fallback to base64 encoding when Firebase Storage is not available
      // Reduce file size limit for base64 storage
      const maxSizeForBase64 = 1 * 1024 * 1024; // 1MB for base64
      if (file.size > maxSizeForBase64) {
        throw new Error('File size too large for current storage. Please upload an image smaller than 1MB.');
      }
    ...
    */
    ...
    if (error instanceof Error) {
      if (error.message.includes('storage/unauthorized')) {
        throw new Error('You do not have permission to upload images. Please try signing in again.');
...
export async function updateProfileImage(userId: string, photoURL: string): Promise<void> {
  try {
    // Update in Firestore
    ...
    if (currentUser && currentUser.uid === userId && !photoURL.startsWith('data:')) {
      await updateProfile(currentUser, { photoURL });
    } else if (photoURL.startsWith('data:')) {
      console.log('Base64 image detected, skipping Firebase Auth update due to URL length limit');
...
export async function deleteProfileImage(imageUrl: string): Promise<void> {
  try {
    ...
    try {
      ...
      console.warn('Could not delete from Firebase Storage (may not be available):', storageError);
