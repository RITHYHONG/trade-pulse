import {
  ...
  Region,
  EventCategory,
} from "../app/calendar/components/economic-calendar/types";
...
export class EconomicCalendarService {
  private static FMP_API_KEY = process.env.FMP_API_KEY;
  private static FINNHUB_API_KEY = process.env.FINNHUB_API_KEY;
  ...
  static async getEvents(
    ...
    try {
      if (this.FMP_API_KEY && !isPlaceholder(this.FMP_API_KEY)) {
        console.log(
          "Fetching economic calendar from Financial Modeling Prep...",
        );
        return await this.fetchFromFMP(startDate, endDate);
      } else {
        console.info(
          "FMP API key not configured or placeholder detected, trying Finnhub...",
        );
      ...
      if (error instanceof Error && error.message !== "MISSING_FMP_KEY") {
        console.warn(
          "FMP API check failed, attempting Finnhub fallback...",
          error.message,
        );
      }
      ...
      if (this.FINNHUB_API_KEY && !isPlaceholder(this.FINNHUB_API_KEY)) {
        try {
          ...
          console.warn(
            ...
            fError instanceof Error ? fError.message : fError,
          );
        }
      }

      console.info(
        "Using high-fidelity Mock Data for economic calendar (API keys not available or failed).",
      );
  ...
  private static async fetchFromFMP(
    startDate: Date,
    endDate: Date,
  ): Promise<EconomicEvent[]> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `FMP API Error ${response.status}: ${response.statusText || errorText.substring(0, 100)}`,
        );
      }
  ...
  private static mapFMPEventToDomain(event: FMPCalendarEvent): EconomicEvent {
    ...
    const id = `${event.event}-${event.date}-${event.country}`
      .replace(/\s+/g, "-")
      .toLowerCase();
    const affectedAssets = EconomicCalendarService.deduceAffectedAssets(
      event.country,
      event.event,
    );
    ...
    return {
      ...
      historicalData: EconomicCalendarService.generateHistoricalData(
        event.event,
        event.impact,
      ),
      ...
      tradingSetup: EconomicCalendarService.generateTradingSetup(
        event.event,
        event.impact,
        affectedAssets,
      ),
  ...
  private static async fetchFromFinnhub(
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text().catch(() => "");
        throw new Error(
          `Finnhub API Error ${response.status}: ${response.statusText || errorText.substring(0, 100)}`,
        );
      }

      const data = await response.json();
      const events: FinnhubCalendarEvent[] = Array.isArray(data)
        ? data
  ...
  private static mapFinnhubEventToDomain(
    event: FinnhubCalendarEvent,
  ): EconomicEvent {
    const id = `${event.event}-${event.time}-${event.country}`
      .replace(/\s+/g, "-")
      .toLowerCase();
    const affectedAssets = EconomicCalendarService.deduceAffectedAssets(
      event.country,
  ...
  private static deduceCategory(eventName: string): EventCategory {
    ...
    if (
      ...
      return "trade";
    if (name.includes("retail") || name.includes("consumption"))
      return "retail";
    if (
      name.includes("pmi") ||
  ...
  private static deduceAffectedAssets(
    country: string,
    eventName: string,
  ): string[] {
    const assets: string[] = [];
    ...
    if (region === "US") {
      ...
      if (
        eventName.toLowerCase().includes("oil") ||
        eventName.includes("inventories")
      )
        assets.push("WTI");
    } else if (region === "EU") {
  ...
  private static generateTradingSetup(
    eventName: string,
    impact: string,
    correlatedAssets: string[],
  ): EconomicEvent["tradingSetup"] {
    const strategies = [
      "Fade the first move",
      "Breakout pending",
